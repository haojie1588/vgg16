...
def main():
    ...
    train_dataset_paths = 'data/train/' # 指定训练集数据路径（根据实际情况指定训练数据集的路径）
    test_dataset_paths = 'data/test/' # 指定测试集数据路径（根据实际情况指定测试数据集的路径）
    ...
    for train_dataset_path in os.listdir(train_dataset_paths):
        a.append(train_dataset_path)
    for i,img in enumerate(a):
        img_path=train_dataset_paths+img
    ...
    for test_dataset_path in os.listdir(test_dataset_paths):
        b.append(test_dataset_path)
    for i,img in enumerate(a):
        img_path=test_dataset_paths+img
    ...
    fc8 = VGG16(X=x, NUM_CLASSES=num_classes)
    ...
    with tf.Session() as sess:
        sess.run(init)
        #saver.restore(sess, "tmp/checkpoints/model.ckpt")
        # Tensorboard
        ...
        for epoch in range(num_epochs):
            ...
            checkpoint_name = './tmp/checkpoints/model_epoch.ckpt'
